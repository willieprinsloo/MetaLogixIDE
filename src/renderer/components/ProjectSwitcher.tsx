...
export function ProjectSwitcher({ open, onClose, onPick }: { open: boolean; onClose: () => void; onPick: (p: Project) => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-start justify-center pt-32 z-50" onClick={onClose}>
      <div className="bg-[--panel-strong] w-[600px] rounded-lg shadow-lg border border-[--border]" onClick={e => e.stopPropagation()}>
        <input
          ref={inputRef}
          value={query}
          ...
          className="w-full bg-transparent p-3 border-b border-[--border] outline-none"
        />
        <ul className="max-h-96 overflow-y-auto">
          {results.slice(0, 20).map(p => (
            <li key={p.id}>
              <button onClick={() => { onPick(p); onClose(); }} className="w-full text-left px-3 py-2 hover:bg-[--panel]">
                <div className="text-sm">{p.name}</div>
                <div className="text-xs text-[--text-muted]">{p.path}</div>
              </button>
            </li>
          ))}
        </ul>
